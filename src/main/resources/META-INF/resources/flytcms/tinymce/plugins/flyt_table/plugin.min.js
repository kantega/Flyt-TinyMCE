...
define("tinymce/tableplugin/Dialogs", [
	...
	return function(editor) {
		...
		function updateStyle(dom, win, isStyleCtrl) {
			...
			if(!data.border || data.border == '0' || data.border == 'false') {
				css["border-color"] = '#fff';
			} else {
				css["border-color"] = undefined;
			}
			data.border = data.border ? '1' : '0';
		...
		self.table = function(isProps) {
					...
					if (editor.settings.table_style_by_css) {
						...
						if (tableElm.children) {
							for (var i = 0; i < tableElm.children.length; i++) {
								styleTDTH(tableElm.children[i], 'border-color', 'inherit');
			...
			if (isProps === true) {
				...
				if (tableElm) {
					var borderValue = dom.getAttrib(tableElm, 'border');
					var border = borderValue == '' || borderValue == '1';
				...
				items: [
					{
						...
						items: (editor.settings.table_appearance_options !== false) ? [
							...
							{label: 'Border', name: 'border', type: 'checkbox'},
...
define("tinymce/tableplugin/Plugin", [
	...
	PluginManager.add('flyt_table', Plugin);
});
})(this);
